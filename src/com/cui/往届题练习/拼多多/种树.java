...
/**
 * 小多想在美化一下自己的庄园。他的庄园毗邻一条小河，他希望在河边种一排树，共 M 棵。小多采购了 N 个品种的树，每个品种的数量
 * 是 Ai (树的总数量恰好为 M)。但是他希望任意两棵相邻的树不是同一品种的。小多请你帮忙设计一种满足要求的种树方案。
 *
 ...
 * 1 <= M <= 2000
 *
 * 输出：输出一行，包含 M 个正整数，分别表示第 i 棵树的品种编号 (品种编号从1到 N)。若存在多种可行方案，
...
public class 种树 {
    ...
    static boolean check(int left) { //left表示剩下多少树没种
        for (int i = 1; i <= n; i++) {
            if (tree[i] > (left + 1) / 2) return false; //若该种树的数量超过未种数量的一半，则不能种这种树
        }
        return true;
    }
    ...
    static boolean dfs(int idx) {
        ...
        if (idx == m) {
            ...
            for (int i = 1; i <= n; i++) {
                if (idx == 0 || (tree[i] != 0 && i != Integer.valueOf(ans.get(idx - 1)))) { //相邻树种不同
                    tree[i]--;
                    ans.add(i + ""); //种树
